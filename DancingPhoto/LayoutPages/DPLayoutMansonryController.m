...
@interface DPLayoutMansonryController ()

@end

@implementation DPLayoutMansonryController

- (void)viewDidLoad {
    self.pageType = kPageTypeMasonry;
    [super viewDidLoad];
    ...
    [colorMatrix enumerateObjectsUsingBlock:^(NSArray * _Nonnull xColors, NSUInteger yIdx, BOOL * _Nonnull stop) {
        ...
        for (NSInteger i = 0; i < xLen; i++) {
            ...
            [dot mas_makeConstraints:^(MASConstraintMaker *make) {
                make.left.mas_equalTo(xStartOffset + (xGap + xStep) * i);
                make.top.mas_equalTo(yStartOffset + yIdx * (yStep + yGap));
                make.width.mas_equalTo(xStep);
                make.height.mas_equalTo(yStep);
            }];
